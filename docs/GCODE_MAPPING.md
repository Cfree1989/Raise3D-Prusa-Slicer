...
| `;Print Time:` / `;Material#N Used:` / per-layer `;PRINTING_TIME:` `;REMAINING_TIME:` | `remaining_times = 1` emits `M73 P… R…` (minutes). `ensure_m99123_first.py` converts those to ideaMaker second counts, injects the first remaining-time block after `M1001`, and appends `;Print Time:` / `;Material#N Used:` from PrusaSlicer's estimated-time and filament-used comments | Copied dialect | RaiseTouch file details and Tune progress read these comments, not `M73` and not the `3h13m` filename. ideaMaker 4.1.0 added this format. |
