...
| `;Print Time:` / `;Material#N Used:` / per-layer `;PRINTING_TIME:` `;REMAINING_TIME:` | `remaining_times = 1` emits `M73 P… R…` (minutes). Post-process drops `M73`, writes second counts on the two lines immediately before each `;LAYER:N` (not `;LAYER_CHANGE`), copies `;Print Time:` into the header (after `;Firmware:`) and again at EOF. If `M73` is missing, remaining time is interpolated from the layer index. | Copied dialect | RaiseTouch Tune reads the comments glued to `;LAYER:N`, matching ideaMaker. Attaching them to PrusaSlicer's `;LAYER_CHANGE` leaves `;Z:` / `;HEIGHT:` in between and the clock never updates. Filename `2h9m` and file-byte progress are not used. File-picker 0s/0g/0$ still comes from the ideaMaker `.data` companion. |
