...
| `;Print Time:` / `;Material#N Used:` / per-layer `;PRINTING_TIME:` `;REMAINING_TIME:` | `remaining_times = 1` emits `M73 P… R…` (minutes). Post-process drops `M73`, writes second counts immediately before each `;LAYER_CHANGE` / `;LAYER:`, copies `;Print Time:` into the header (after `;Firmware:`) and again at EOF | Copied dialect | RaiseTouch Tune reads layer time comments, not filename `2h9m` and not file-byte progress. A lone block after `M1001` is ignored during heat-up (0.2% of the file → ~17 h remaining). File-picker 0s/0g/0$ still comes from the ideaMaker `.data` companion; G-code comments may fill it if the UI scans the header. |
