...
2. Run (required: PrusaSlicer prepends `; generated by …` and emits `M204 S` for accel; this script puts `M99123` on line 1 and rewrites `M204` to `SET_VELOCITY_LIMIT`):
...
Optional: Print Settings → Output options → Post-processing scripts → `python` plus the full path to `scripts\ensure_m99123_first.py` in this repo ([PrusaSlicer post-processing](https://help.prusa3d.com/article/post-processing-scripts_283913)). That both moves `M99123` and converts `M204`. G-code thumbnails are off so a PNG block is not sitting in front of the header.
