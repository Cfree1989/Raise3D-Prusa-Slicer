...
4. Filament: PLA / PETG / TPU / ASA / PA-CF / ABS-GF Raise3D (G-code name `[Raise3D] ` + `filament_type`). PLA is 215 °C first / 225 °C later / 0.94 flow (ideaMaker `[Raise3D] PLA` files used 230 °C). Other materials keep the operator `* XL` temps, cooling, and PETG/TPU retract. Dual standby 180 °C. Volumetric `min(XL, 15 mm³/s)` (TPU 2.5, PA-CF 8). Compatible only with this Dual 0.4 printer.
5. Print: one **0.20mm Hyper Speed** profile from this machine’s ideaMaker jobs (0.20 mm / 0.30 mm first / 50 mm/s first / walls 150 / infill 120 / tops 100 / travel 150 / accel 2000/5000). PLA 215/225 °C / 0.94 flow. `ensure_m99123_first.py` converts `M204 S` to `SET_VELOCITY_LIMIT`. Wipe tower on — default X50 Y140 (past T1 keep-out); relative E / `M83` (ideaMaker used `M82`).
6. Left-only from `LeftonlyExtruder.gcode`. Right-only from `RightonlyExtruder.gcode` (T1 home + same `X80 Y0` wipe). Dual start, standby tool-change, and dual end from `MulticolorRaise3d.gcode`. After purge, first print travel is XY at Z15 then Z (ideaMaker). Single-tool end matches the matching ideaMaker file.
...
- ideaMaker dual-file skirt at 15 mm/s
- ideaMaker 50→150 speed ramp on layers 0–4 (PrusaSlicer first-layer speed only)
