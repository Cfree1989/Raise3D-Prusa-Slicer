...
4. Filament: PLA / PETG / TPU / ASA / PA-CF / ABS-GF Raise3D (G-code name `[Raise3D] ` + `filament_type`). Temps, cooling, and PETG/TPU retract from the operator’s matching `* XL` user presets. PLA also matches the proven Prusa PLA. Dual standby 180 °C. Volumetric `min(XL, 15 mm³/s)` (TPU 2.5, PA-CF 8). Compatible only with this Dual 0.4 printer.
5. Print: XL-style 0.4 mm family (0.10 FAST DETAIL, 0.15/0.20/0.25 SPEED and STRUCTURAL, 0.28 DRAFT). Default 0.20 mm SPEED. Motion clipped to Pro2 Hyper FFF L1 (150 mm/s, 5000 mm/s², 15 mm³/s). Travel 150 mm/s XY, 12 mm/s Z. Accel: print 2000 (ideaMaker) / travel 5000 (ideaMaker) / first layer 500 and short-travel 250 (XL). First layer 40 mm/s walls / 100 mm/s infill on every profile. `ensure_m99123_first.py` converts `M204 S` to `SET_VELOCITY_LIMIT`. Wipe tower on — default X50 Y140 (past T1 keep-out); relative E / `M83` (ideaMaker used `M82`).
...
- ideaMaker first layer 0.30 mm, skirt, and 15 mm/s first-layer feed (print family stays XL: 0.20 mm, no skirt, 40 mm/s first-layer walls / 100 mm/s infill, 500 mm/s² first-layer accel)
